...
@pytest.mark.skipif(not is_gpu, reason="FlashAttention requires CUDA or XPU")
def test_masked_cross_attention_matches_sdpa():
    if not HAS_FLASH_ATTN:
        pytest.skip("No Flash Attention available")

    device = torch.device(current_omni_platform.device_type)
    dtype = torch.bfloat16
    batch_size, query_length, key_length = 2, 17, 9
    num_heads, head_dim = 4, 64
    softmax_scale = 1.0 / (head_dim**0.5)

    torch.manual_seed(211)
    query = torch.randn(batch_size, query_length, num_heads, head_dim, device=device, dtype=dtype)
    key = torch.randn(batch_size, key_length, num_heads, head_dim, device=device, dtype=dtype)
    ...
    key = key.masked_fill(invalid_keys, 100.0)
    value = value.masked_fill(invalid_keys, -100.0)
    ...
    actual = flash.forward(query.clone(), key.clone(), value.clone(), metadata)
    expected = sdpa.forward(query.clone(), key.clone(), value.clone(), metadata)
...
@pytest.mark.parametrize("query_length", [5, 7])
def test_masked_cross_attention_unpads_only_kv(monkeypatch, query_length):
    calls = []
    ...
    batch_size, key_length, num_heads, head_dim = 2, 5, 2, 4
    query = torch.arange(
        batch_size * query_length * num_heads * head_dim,
        dtype=torch.float32,
    ).reshape(batch_size, query_length, num_heads, head_dim)
    key = torch.arange(
        batch_size * key_length * num_heads * head_dim,
        dtype=torch.float32,
    ).reshape(batch_size, key_length, num_heads, head_dim)
    value = key + 1000
    mask = torch.tensor(
        [
    ...
    assert torch.equal(
        kwargs["cu_seqlens_q"],
        torch.tensor([0, query_length, 2 * query_length], dtype=torch.int32),
    )
    assert torch.equal(kwargs["cu_seqlens_k"], torch.tensor([0, 3, 7], dtype=torch.int32))
    assert kwargs["max_seqlen_q"] == query_length
...
@pytest.mark.parametrize(
    ("mask", "error_type", "match"),
    [
        (torch.ones(2, 5), TypeError, "dtype torch.bool"),
        (torch.ones(2, 1, 1, 5, dtype=torch.bool), ValueError, "must be 2D"),
        (torch.ones(2, 4, dtype=torch.bool), ValueError, "shape must match"),
        (
...
def test_flash_attention_rejects_noncanonical_padding_masks(monkeypatch, mask, error_type, match):
    monkeypatch.setattr(fa, "HAS_FLASH_ATTN", True)
    impl = FlashAttentionImpl(
        num_heads=2,
        head_size=4,
    ...
    query = torch.randn(2, 7, 2, 4)
    key = torch.randn(2, 5, 2, 4)

    with pytest.raises(error_type, match=match):
        impl.forward_cuda(query, key, key, AttentionMetadata(attn_mask=mask))


def test_all_valid_cross_attention_mask_uses_dense_path(monkeypatch):
    dense_calls = []

    def fake_dense_func(q, k, v, **kwargs):
        dense_calls.append((q.shape, k.shape, kwargs))
        return q

    def unexpected_varlen_func(*args, **kwargs):
        raise AssertionError("all-valid cross-attention mask should not use varlen")

    monkeypatch.setattr(fa, "HAS_FLASH_ATTN", True)
    monkeypatch.setattr(fa, "flash_attn_func", fake_dense_func)
    monkeypatch.setattr(fa, "flash_attn_varlen_func", unexpected_varlen_func)
    impl = FlashAttentionImpl(
        num_heads=2,
        head_size=4,
        softmax_scale=0.5,
    ...
    output = impl.forward_cuda(
        ...
        key,
        key,
        AttentionMetadata(attn_mask=torch.ones(2, 5, dtype=torch.bool)),
    )

    assert torch.equal(output, query)
    assert len(dense_calls) == 1
    assert dense_calls[0][:2] == (query.shape, key.shape)
